...
const char call_r8[] = "A\xff\xd0"; // call r8
...
int main()
{
    ...
    while (sc_idx < nr) {
        // syscall
        if (!memcmp(shellcode+sc_idx, syscall_pattern, sizeof(syscall_pattern)-1)) { // if memcmp return 0: both same
        ...
        // call <reg>
        int i = 0;
        for (; i < REG_CNT; i++) { // CNT=14, search all reg pattern, if match, exec that pattern and break.
        ...
        if (i < REG_CNT) // if there is a match, return to while loop
            continue;
