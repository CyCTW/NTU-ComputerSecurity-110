...
if sys.argv[1] == '1':
    ...
    #    - name: 0xdeadbeef
    #    - bark: system
    # Note: 因為大小為0x28 (+0x08) 因此會取到tcache 0x30裡的第一個: aminal[1]
    # buy會做兩次malloc，第一次animal[1]會拿到舊的animal[1]，第二次animal[1]->name會拿到舊的animal[0]
    # 之後可以蓋掉animal[0]的bark function pointer來拿到shell
    ...
    #    - bark: one gadget
    # Note: 這行寫入animal[0]->name的動作，其實是寫到新的name address (heap+0xbe0) (animal[1])
    # 因為buy(1, 0x10, dummy)的animal[1]找不到符合大小(0x30)的bin，因此會自己創一個，animal[1]的name可以用舊0x20大小 的tcache bin 
    # 因此我們要hiject的位址為新的animal[1]的位址
